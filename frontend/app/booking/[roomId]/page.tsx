...
import { bookingApi, roomTypeApi } from '@/lib/api';
import { RoomTypeGlobalResponse } from '@/types';
...
function BookingForm() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.push(`/login?returnUrl=${encodeURIComponent(`/booking/${roomTypeId}`)}`);
      return;
    }
    setRoomTypeLoading(true);
    roomTypeApi.getById(roomTypeId)
      .then(setRoomType)
      .catch(() => setRoomType(null))
      .finally(() => setRoomTypeLoading(false));
  }, [user, authLoading, roomTypeId, router]);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      const booking = await bookingApi.create({ roomTypeId, checkIn, checkOut });
  ...
  if (authLoading || roomTypeLoading) return <LoadingScreen />;
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    <h2 className="font-bold text-gray-900 truncate">{roomType.name}</h2>
                    <p className="text-xs text-gray-500 truncate">{roomType.hotelName}</p>
                  </div>
                </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          {success ? (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-gray-500 mt-1">Your room type has been reserved successfully.</p>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="mt-1 text-sm text-gray-700">
                        {nights} night{nights !== 1 ? 's' : ''} &middot; ${roomType.price} / night
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-2xl font-bold text-gray-900">
                        ${(roomType.price * nights).toFixed(2)}
                      </div>
