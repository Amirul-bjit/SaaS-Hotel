...
export default function OwnerNewRoomPage() {
    ...
    async function fetchData() {
      try {
        ...
        if (!sub.isActive) {
          ...
          if (rooms.length >= sub.planConfig.maxRooms) {
            setBlockReason(
              `You have reached the maximum room limit for your ${sub.planType} plan (${rooms.length}/${sub.planConfig.maxRooms} rooms). Upgrade your plan to add more rooms.`
            );
          }
        }
      } catch {
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (!user?.hotelId || !selectedRoomTypeId) return;
    setError('');
    setLoading(true);
    try {
  ...
  return (
    <div className="px-6 py-8 max-w-lg">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">Create a new physical room unit for your hotel</p>
      </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              <div>
                ...
                >
                  ...
                  {roomTypes.map((rt) => (
                    <option key={rt.id} value={rt.id}>{rt.name} — ${rt.price}/night, up to {rt.maxGuests} guests</option>
                  ))}
                </select>
              </div>
              <Input label="Room Number" value={roomNumber} onChange={(e) => setRoomNumber(e.target.value)} required placeholder="e.g. 101" />
